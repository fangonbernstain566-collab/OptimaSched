...
const JWT_SECRET =
  ...
  'replace_this_with_a_long_secure_random_string_for_dev';
  ...
  try {
    ...
    if (existingUser) {
      return res.status(400).json({
        success: false,
        error: 'An account with that email identifier already exists.',
      });
    }
    ...
    if (!targetRole) {
      return res.status(400).json({
        success: false,
        error: `System role context '${roleName}' does not exist.`,
      });
    }
  ...
  try {
    ...
    if (!user) {
      return res.status(401).json({
        success: false,
        error: 'Invalid cryptographic credentials mismatch.',
      });
    }
